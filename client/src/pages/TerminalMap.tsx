...
function makeHeatBlob(colorHex: string, rx: number, rz: number, peakOpacity: number, innerRatio = 1.0): THREE.Mesh {
  ...
  g.addColorStop(0.45, `${colorHex}bb`);
  g.addColorStop(0.70, `${colorHex}66`);
  g.addColorStop(0.90, `${colorHex}22`);
...
class TerminalScene {
  ...
  applyData(
    ...
    heatGroups.forEach(({ cx, cz, bw, bd, conc, isMax }) => {
      const col = isMax ? "#ff0000" : conc === "High" ? "#fc6203" : conc === "Medium" ? "#fcb900" : "#008000";
      const spread = isMax ? 2.6 : conc === "High" ? 2.5 : conc === "Medium" ? 2.2 : 1.6;
      const peakOp = isMax ? 1.0 : conc === "High" ? 0.90 : conc === "Medium" ? 0.80 : 0.70;
      ...
      addBlob(bw * spread, bd * spread, peakOp * 0.65, 0.06);
      ...
      if (isMax || conc === "High" || conc === "Medium") {
        const pCount = isMax ? 100 : conc === "High" ? 60 : 30;
        const maxH = isMax ? 4.0 : conc === "High" ? 2.5 : 1.4;
