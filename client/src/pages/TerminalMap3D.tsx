...
const TERM_W = 1000, TERM_D = 680, TERM_CX = 480, TERM_CY = 410;
...
// Properly scaled layout mapping identical to the 2D layout constraints
...
const REALISTIC_CONTAINERS = [
  0x1e293b, 0x0f172a, 0xca8a04, 0x991b1b,
  0x0369a1, 0x064e3b, 0xd97706, 0xe2e8f0,
];
...
class TerminalScene {
  ...
  buildGroundSlabs() {
    ...
    const createSlab = (startX: number, endX: number, startY: number, endY: number, material: THREE.Material) => {
      const w = (endX - startX) * S, d = (endY - startY) * S;
      ...
      slab.position.set(cx, -1.1, cz); slab.receiveShadow = true; this.scene.add(slab);
      if (material === slabMat) {
        const surf = new THREE.Mesh(new THREE.BoxGeometry(w, 0.04, d), surfMat);
        surf.position.set(cx, 0.02, cz); surf.receiveShadow = true; this.scene.add(surf);
      }
    };
    ...
    trenchFloor.receiveShadow = true; this.scene.add(trenchFloor);

    const stripeMat = new THREE.MeshBasicMaterial({ color: 0xf5b800, transparent: true, opacity: 0.7 });
    const northZ = to3D(TERM_CX, EDGE_N).z, southZ = to3D(TERM_CX, EDGE_S).z;
    const eastX = to3D(EDGE_E, TERM_CY).x, cx3 = to3D(TERM_CX, 0).x;
    ...
    [northZ + 0.6, southZ - 0.6].forEach(z => {
      ...
      drain.position.set(cx3, 0.0, z); this.scene.add(drain);
    });
    ...
    [northZ - 0.05, southZ + 0.05].forEach(z => {
      for (let i = 0; i < fenderCount; i++) {
        ...
        fender.castShadow = true; this.scene.add(fender);
      }
    });
    ...
    [[EDGE_N, northZ], [EDGE_S, southZ]].forEach(([, z]) => {
      for (let i = 0; i < 10; i++) {
        ...
        b.position.set(bx, 0.27, z as number); b.castShadow = true; this.scene.add(b);
        const cap = new THREE.Mesh(bollardCapGeo, bollardMat);
        cap.position.set(bx, 0.58, z as number); this.scene.add(cap);
      }
    });
    void concreteMat;
  }
  ...
  buildYardMarkings() {
    ...
    // Grid mapping exactly matching the dynamic block positioning bounds
    for (let py = 0; py < 8; py++) {
      for (let px = 0; px < 10; px++) {
        ...
        outline.position.set(cx, 0.05, cz); this.scene.add(outline);

        const slotW = bw / 7;
        for (let s = 1; s < 7; s++) {
          const sl = new THREE.Mesh(new THREE.PlaneGeometry(0.015, bd * 0.9), lineMat);
          sl.rotation.x = -Math.PI / 2;
          sl.position.set(cx - bw / 2 + s * slotW, 0.05, cz); this.scene.add(sl);
        }
      }
    }

    const hLanes = [130, 410, 620];
    hLanes.forEach(y => {
      const lz = to3D(0, y).z;
      for (let i = 0; i < 40; i++) {
    ...
    const slowMat = new THREE.MeshBasicMaterial({ map: slowTex, transparent: true, opacity: 0.65, depthWrite: false });
    hLanes.forEach(y => {
      for (let i = 0; i < 3; i++) {
  ...
  buildFuelStation() {
    ...
    g.position.set(pos3.x, 0, pos3.z); this.scene.add(g);
    const stain = new THREE.Mesh(new THREE.PlaneGeometry(2.5, 1.8),
      new THREE.MeshBasicMaterial({ color: 0x111827, transparent: true, opacity: 0.25 }));
    stain.rotation.x = -Math.PI / 2;
    stain.position.set(pos3.x, 0.03, pos3.z); this.scene.add(stain);
  }
  ...
  buildGateComplex() {
    ...
    gateSign.position.set(0, 1.4, 0.031); g.add(gateSign);
    g.position.set(pos3.x, 0, pos3.z); this.scene.add(g);
  }
  ...
  buildArchitecture() {
    ...
    [320, 580].forEach((y, i) => {
      ...
      whSign.position.set(0, 1.25, 1.501); wh.add(whSign);
      wh.position.set(to3D(-350, y).x, 0, to3D(-350, y).z); this.scene.add(wh);
      this.buildParkedRTG(to3D(-280, y).x, to3D(-280, y).z);
    });

    for (let i = 0; i < 45; i++) {
      const rx = -100 - Math.random() * 500, ry = 100 + Math.random() * 600;
    ...
    const vRoads = [140, 340, 540, 740, 940], hRoads = [130, 410, 620];
    hRoads.forEach(y => {
    ...
    vRoads.forEach(x => {
      if (x <= TRENCH_END_X) {
        ...
        r.position.set(to3D(x, TERM_CY).x, 0.045, to3D(x, TERM_CY).z); this.scene.add(r);
      }
    });
    ...
    const ballast = new THREE.Mesh(new THREE.BoxGeometry(trenchWStr, 0.08, 20 * S),
      new THREE.MeshStandardMaterial({ color: 0x3f3f46, roughness: 0.9 }));
    ballast.position.set(trenchCXStr, TRENCH_Y + 0.04, railZ); this.scene.add(ballast);
    const matSleeper = new THREE.MeshStandardMaterial({ color: 0x292524 });
    const startSleeperX = to3D(TRENCH_START_X, 0).x, endSleeperX = to3D(TRENCH_END_X, 0).x;
    ...
    for (let i = 0; i < sleeperCount; i++) {
      ...
      sleeper.position.set(startSleeperX + i * 0.2, TRENCH_Y + 0.09, railZ); this.scene.add(sleeper);
    }
    ...
    const r2 = r1.clone(); r2.position.set(trenchCXStr, TRENCH_Y + 0.12, railZ + 0.15); this.scene.add(r2);
  ...
  buildTrain(zPos: number) {
    ...
    [-0.3, 0.3].forEach(wx => {
      [0.1, -0.1].forEach(woff => {
        this.createWheel(wx + woff, 0.05, 0.15, locoGroup, "train");
        this.createWheel(wx + woff, 0.05, -0.15, locoGroup, "train");
      });
    });
    locoGroup.position.set(0.8, 0, 0); train.add(locoGroup);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      [-0.22, 0.22].forEach(wx => {
        [0.08, -0.08].forEach(woff => {
          this.createWheel(wx + woff, 0.05, 0.15, carGroup, "train");
          this.createWheel(wx + woff, 0.05, -0.15, carGroup, "train");
        });
      });
      if (i !== 1 && Math.random() > 0.2) {
        const cont = new THREE.Mesh(new THREE.BoxGeometry(0.60, 0.22, 0.23),
          new THREE.MeshStandardMaterial({ color: REALISTIC_CONTAINERS[i % REALISTIC_CONTAINERS.length], roughness: 0.7 }));
        cont.position.set(0, 0.23, 0); carGroup.add(cont);
      }
      carGroup.position.set(-i * 0.7, 0, 0); train.add(carGroup);
    }

    const startX = to3D(TRENCH_START_X + 100, 0).x;
    train.position.set(startX, trainY, zPos);
    train.userData = { state: "INBOUND", speed: 0.05, targetX: this.rmg.craneX + 0.7, startX, stopTime: 0 };
    this.scene.add(train); this.train = train;
  }
  ...
  buildRMGCrane(xPos: number) {
    ...
    const z1 = to3D(0, 450).z, z2 = to3D(0, 530).z;
    [-0.3, 0.3].forEach(dx => {
    ...
    const bridgeZCenter = (z1 + z2) / 2, bridgeLength = Math.abs(z1 - z2) + 0.4;
    const bridge = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.2, bridgeLength), steel);
    bridge.position.set(0, 1.8, bridgeZCenter); craneGroup.add(bridge);
    ...
    this.rmg.heldContainer = new THREE.Mesh(new THREE.BoxGeometry(0.60, 0.22, 0.23),
  ...
  buildTrucks() {
    ...
    const pathOuter = [getP(40, 130), getP(910, 130), getP(910, 630), getP(40, 630)];
    const pathInnerW = [getP(150, 130), getP(350, 130), getP(350, 410), getP(150, 410)];
    const pathInnerE = [getP(550, 410), getP(750, 410), getP(750, 630), getP(550, 630)];
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (Math.random() > 0.2) {
        const cont = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.22, 0.48),
          new THREE.MeshStandardMaterial({ color: REALISTIC_CONTAINERS[i % REALISTIC_CONTAINERS.length], roughness: 0.6 }));
        cont.position.set(0, 0.21, -0.28); trailer.add(cont);
      }
      truck.add(trailer);
      ...
      this.trucks.push({ mesh: truck, path, targetIdx: nextIdx, speed: 0.015 + Math.random() * 0.008 });
    }
  }
  ...
  buildDocks() {
    ...
    nw.castShadow = true; nw.receiveShadow = true; this.scene.add(nw);
    const sw = nw.clone();
    sw.position.set(c.x, 0.17, to3D(TERM_CX, EDGE_S).z + 0.5); this.scene.add(sw);
    ...
    ew.castShadow = true; ew.receiveShadow = true; this.scene.add(ew);
    ...
    const northZ = to3D(TERM_CX, EDGE_N).z - 0.9, southZ = to3D(TERM_CX, EDGE_S).z + 0.9;
    const eastX = to3D(EDGE_E, TERM_CY).x + 0.9;
    [-12, -8, -5, -2, 1, 4, 7].forEach(x => {
      [northZ, southZ].forEach(z => {
        const b = new THREE.Mesh(bGeo, bMat); b.position.set(x, 0.4, z); b.castShadow = true; this.scene.add(b);
      });
    });
    [-8, -5, -2, 1, 4, 7].forEach(z => {
      const b = new THREE.Mesh(bGeo, bMat); b.position.set(eastX, 0.4, z); b.castShadow = true; this.scene.add(b);
    });
    ...
    [c.x - TERM_W * S / 2, southZ], [c.x + TERM_W * S / 2, southZ]].forEach(([lx, lz]) => {
      ...
      navLight.position.set(lx as number, 0.8, lz as number); this.scene.add(navLight);
      const glow = new THREE.PointLight(0xff4400, 1.5, 3);
      glow.position.set(lx as number, 0.8, lz as number); this.scene.add(glow);
    });
    ...
    [-10, -5, 0, 5].forEach(lx => {
      for (let r = 0; r < 4; r++) {
        ...
        rung.position.set(lx, 0.35 - r * 0.12, northZ - 0.02); this.scene.add(rung);
      }
    });
  }
  ...
  buildSTSCrane(svgX: number, svgY: number, rotDeg: number) {
    ...
    [-0.55, 0.55].forEach(dz => {
      ...
      [-0.2, 0.2].forEach(wx => {
        const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.06, 8).rotateX(Math.PI / 2), MAT_WHEEL);
        wheel.position.set(wx, 0.05, dz); g.add(wheel);
      });
    });
    ...
    [1.0, 1.8].forEach(y => {
      const brace = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.06, 0.65), steel);
      brace.position.set(-0.07, y, 0); g.add(brace);
    });
    ...
    [0xf59e0b, 0x1a56c4, 0xf59e0b].forEach((col, si) => {
  ...
  buildShip(id: string, svgX: number, svgY: number, rotDeg: number, name: string, isTarget: boolean) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      plate.position.y = -DR / 2 + (i + 1) * (DR / 6); g.add(plate);
    }
    ...
    g.add(new THREE.Mesh(bGeo, hullMat));
    const deck = new THREE.Mesh(new THREE.BoxGeometry(L * 0.94, 0.06, W * 0.88),
    ...
    for (let h = 0; h < 5; h++) {
      const hatch = new THREE.Mesh(new THREE.BoxGeometry(L * 0.13, 0.04, W * 0.75), hatchMat);
      hatch.position.set(-L * 0.38 + h * L * 0.19, DR / 2 + 0.06, 0); g.add(hatch);
    }
    const COLS = 13, ROWS = 3;
    const cW = (L * 0.72) / COLS, cD = (W * 0.80) / ROWS;
    for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) {
      const stackH = 0.14 + Math.sin((c / COLS) * Math.PI) * 0.20 + Math.random() * 0.08;
      const cm = new THREE.Mesh(new THREE.BoxGeometry(cW * 0.88, stackH, cD * 0.88),
        new THREE.MeshStandardMaterial({ color: REALISTIC_CONTAINERS[(r * COLS + c) % REALISTIC_CONTAINERS.length], roughness: 0.65 }));
    ...
    super1.position.set(-L * 0.39, DR / 2 + 0.55, 0); super1.castShadow = true; g.add(super1);
    ...
    for (let fl = 0; fl < 3; fl++) for (let w2 = 0; w2 < 4; w2++) {
      const win = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.07, 0.1), winMat);
      win.position.set(-L * 0.39 - 0.43, DR / 2 + 0.25 + fl * 0.3, -W * 0.28 + w2 * 0.2); g.add(win);
    }
    const funnel = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.13, 0.5, 8),
      new THREE.MeshStandardMaterial({ color: isTarget ? 0x1d4ed8 : 0x7f1d1d, roughness: 0.5 }));
    funnel.position.set(-L * 0.42, DR / 2 + 1.35, 0); funnel.castShadow = true; g.add(funnel);
    ...
    [L * 0.3, -L * 0.15].forEach(mx => {
      ...
      mast.position.set(mx, DR / 2 + 0.7, 0); g.add(mast);
    });
    if (isTarget) {
      const ringR = Math.max(L, W) * 0.62;
      const ring = new THREE.Mesh(new THREE.RingGeometry(ringR, ringR + 0.12, 72),
    ...
    label.position.set(0, DR / 2 + 2.4, 0); label.rotation.x = -Math.PI / 7; g.add(label);
    const hullLabel = makeLabel(name, 18, "#aab8c8");
    hullLabel.position.set(L * 0.1, 0, W / 2 + 0.01); hullLabel.rotation.y = -Math.PI / 2; g.add(hullLabel);
    g.position.set(pos.x, 0, pos.z);
    g.rotation.y = THREE.MathUtils.degToRad(-rotDeg);
    g.userData = { type: "ship", id, bobOffset: Math.random() * Math.PI * 2 };
    this.scene.add(g); this.shipMeshes.set(id, g);
  }
  ...
  applyData(
    ...
    const heatGroups: { cx: number; cz: number; conc: string; isMax: boolean }[] = [];

    Object.entries(data?.layout || {}).forEach(([id, pos]: [string, { x: number; y: number }]) => {
      const px = Math.max(0, pos.x), py = Math.max(0, pos.y);
      const svgX = BLK_START_X + px * (BLK_W + BLK_GAP_X);
      const svgY = BLK_START_Y + py * (BLK_H + BLK_GAP_Y);
      const wp = to3D(svgX + BLK_W / 2, svgY + BLK_H / 2);
      const blk = (data?.blocks || {})[id];
      const isMax = id === computedMaxBlock;
      const isRec = typeof recRaw === "string" ? recRaw.includes(id) : Array.isArray(recRaw) ? (recRaw as string[]).includes(id) : false;
      ...
      const pad = new THREE.Mesh(new THREE.BoxGeometry(bw, 0.12, bd),
        new THREE.MeshStandardMaterial({ color: padColor, roughness: 0.92 }));
      pad.position.y = 0.06; pad.castShadow = true; pad.receiveShadow = true; g.add(pad);

      if (hasData) {
        heatGroups.push({ cx: wp.x, cz: wp.z, conc, isMax });
        const count = Math.min(blk.count, 60);
        const COLS = 7, cubeSize = bw * 0.11;
        const dummy = new THREE.Object3D();
        const iMesh = new THREE.InstancedMesh(new THREE.BoxGeometry(cubeSize, cubeSize, cubeSize),
          new THREE.MeshStandardMaterial({ roughness: 0.7 }), count);
        iMesh.castShadow = true;
        for (let i = 0; i < count; i++) {
          const tier = Math.floor(i / (COLS * 5)), indexInTier = i % (COLS * 5);
          const row = Math.floor(indexInTier / COLS), col = indexInTier % COLS;
          dummy.position.set(-bw * 0.36 + col * cubeSize * 1.15, 0.14 + tier * cubeSize + cubeSize / 2, -bd * 0.30 + row * cubeSize * 1.15);
        ...
        iMesh.instanceMatrix.needsUpdate = true; g.add(iMesh);
      }

      const badge = makeLabel(id, 26, isMax ? "#b91c1c" : isRec ? "#0369a1" : "#334155");
      badge.rotation.x = -Math.PI / 2; badge.position.set(-bw * 0.32, 0.20, -bd * 0.34); g.add(badge);
      if (hasData) {
        const cntLabel = makeLabel(`${blk.count}`, 24, "#0f172a");
        cntLabel.rotation.x = -Math.PI / 2; cntLabel.position.set(bw * 0.30, 0.20, -bd * 0.34); g.add(cntLabel);
      }

      g.position.set(wp.x, 0, wp.z);
      g.userData = { type: "block", id, count: blk?.count ?? 0, concentration: conc };
      this.scene.add(g); this.blockMeshes.set(id, g);
    });

    // Consolidated unified single heat map generation using dynamically generated canvas texture
    if (heatGroups.length > 0) {
      ...
      heatGroups.forEach(({ cx, cz, conc, isMax }) => {
        // Compute position inside the canvas spanning the entire terminal
        const svgX = (cx / S) + OX;
        const svgY = (cz / S) + OY;
        const startX = TERM_CX - TERM_W / 2;
        const startY = TERM_CY - TERM_D / 2;

        const px = ((svgX - startX) / TERM_W) * 1024;
        const py = ((svgY - startY) / TERM_D) * 1024;

        // Make blob massive enough to blend seamlessly
        const radius = isMax ? 250 : conc === "High" ? 180 : conc === "Medium" ? 130 : 90;
        const col = isMax ? "239,68,68" : conc === "High" ? "249,115,22" : conc === "Medium" ? "234,179,8" : "34,197,94";
      ...
      const planeMat = new THREE.MeshBasicMaterial({
        map: tex, transparent: true, opacity: 0.9, depthWrite: false, side: THREE.DoubleSide
      });
      const plane = new THREE.Mesh(new THREE.PlaneGeometry(TERM_W * S, TERM_D * S), planeMat);
      plane.rotation.x = -Math.PI / 2;
      plane.position.set(to3D(TERM_CX, TERM_CY).x, 0.08, to3D(TERM_CX, TERM_CY).z);
      this.scene.add(plane);
      this.heatBlobs.push({ mesh: plane, baseOp: 0.9 });
    }
  ...
  animate() {
    ...
    if (this.waterMesh) {
      ...
      pos.needsUpdate = true; this.waterMesh.geometry.computeVertexNormals();
    }
    ...
    // Heat blob pulse effect
    this.heatBlobs.forEach(({ mesh, baseOp }) => {
      (mesh.material as THREE.MeshBasicMaterial).opacity = baseOp * (0.85 + Math.sin(t * 1.5) * 0.15);
    });
    ...
    this.particleSystems.forEach(pts => {
      ...
      for (let i = 0; i < pos.count; i++) {
        ...
        if (y > maxH) { y = 0; pos.setX(i, cx + (Math.random() - 0.5) * bw * 2.0); pos.setZ(i, cz + (Math.random() - 0.5) * bd * 2.0); }
        pos.setY(i, y);
      }
    ...
    this.trucks.forEach(truck => {
      ...
      if (truck.mesh.position.distanceTo(targetVec) < truck.speed * 1.5)
        truck.targetIdx = (truck.targetIdx + 1) % truck.path.length;
      const dir = targetVec.clone().sub(truck.mesh.position).normalize();
      truck.mesh.position.addScaledVector(dir, truck.speed);
      truck.mesh.lookAt(truck.mesh.position.clone().add(dir));
    });

    // Reversing train wheel rotation mappings specifically for INBOUND/OUTBOUND direction inversion 
    if (this.train.userData.state === "INBOUND") {
      ...
      if (this.train.position.x >= this.train.userData.targetX) {
        ...
        this.train.userData.state = "STOPPED"; this.train.userData.stopTime = t;
      }
    } else if (this.train.userData.state === "STOPPED") {
      const elapsed = t - this.train.userData.stopTime;
      if (elapsed > 60) { this.train.userData.state = "OUTBOUND"; this.rmg.heldContainer.visible = false; }
      else {
        const localT = elapsed % 10, progress = localT / 10;
        const trainZ = to3D(0, TRENCH_CY).z, roadZ = to3D(0, 450).z;
        const upY = -0.1, downY = -1.2;
        if (progress < 0.2) { this.rmg.trolley.position.z = THREE.MathUtils.lerp(roadZ, trainZ, progress / 0.2); this.rmg.spreader.position.y = upY; this.rmg.heldContainer.visible = false; }
        else if (progress < 0.3) { this.rmg.spreader.position.y = THREE.MathUtils.lerp(upY, downY, (progress - 0.2) / 0.1); }
        else if (progress < 0.4) { this.rmg.spreader.position.y = THREE.MathUtils.lerp(downY, upY, (progress - 0.3) / 0.1); this.rmg.heldContainer.visible = true; }
        else if (progress < 0.6) { this.rmg.trolley.position.z = THREE.MathUtils.lerp(trainZ, roadZ, (progress - 0.4) / 0.2); }
        else if (progress < 0.7) { this.rmg.spreader.position.y = THREE.MathUtils.lerp(upY, downY, (progress - 0.6) / 0.1); }
        else if (progress < 0.8) { this.rmg.spreader.position.y = THREE.MathUtils.lerp(downY, upY, (progress - 0.7) / 0.1); this.rmg.heldContainer.visible = false; }
      }
    } else if (this.train.userData.state === "OUTBOUND") {
      this.train.position.x -= this.train.userData.speed;
      this.trainWheels.forEach(w => (w.rotation.z -= this.train.userData.speed * 5));
      if (this.train.position.x <= this.train.userData.startX) this.train.userData.state = "INBOUND";
    }

    // Fixed Truck wheel reverse rotation
    ...
    >
      ...
      <Box
        sx={{
          ...
          px: 1.8, py: 1.2,
          bgcolor: mode === "dark" ? "rgba(42,42,42,0.94)" : "rgba(233,238,246,0.96)",
      ...
      {hoveredBlock && (
        <Box
          sx={{
            ...
            px: 2, py: 1.4,
            bgcolor: mode === "dark" ? "rgba(42,42,42,0.97)" : "rgba(233,238,246,0.97)",
          ...
          {hoveredData && (
            <>
              <Typography sx={{ fontSize: "0.66rem", color: "text.primary", mt: 0.4 }}>
                Volume:{" "}
                <span style={{ color: theme.palette.info.main }}>{hoveredData.count} CTN</span>
              </Typography>
              <Typography sx={{ fontSize: "0.66rem", color: "text.primary" }}>
                Density:{" "}
                <span style={{ color: theme.palette.info.main }}>{hoveredData.concentration}</span>
              </Typography>
            </>
          )}
        </Box>
