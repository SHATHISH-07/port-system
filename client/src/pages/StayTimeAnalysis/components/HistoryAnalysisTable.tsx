...
function sortRows(rows: RowData[], orderBy: keyof RowData, order: "asc" | "desc") {
...
export default function HistoryAnalysisTable({
  ...
  actualVisits: Record<string, VisitData>;
  ...
  const rows = useMemo(() => {
    ...
    (assignments || []).forEach((a: VisitData) => {
      if (a?.visit_id) assignmentMap.set(a.visit_id, a);
    });

    const visitRows = Object.entries(actualVisits || {}).map(
      ([visitId, visit]: [string, VisitData]) => {
        ...
        return {
          ...
          discharged: Number(
            visit?.discharged_containers ?? assignment?.discharged ?? 0,
          ),
          restowCount: Number(
            visit?.restow_count ?? assignment?.restow_count ?? 0,
          ),
          assignedCranes: Number(
            visit?.assigned_cranes ?? assignment?.crane_count ?? 0,
          ),
  ...
  return (
    ...
    >
      ...
      <TableContainer
        sx={{ maxHeight: 400, borderTop: `1px solid ${theme.palette.divider}` }}
      >
        <Table
          size="small"
          stickyHeader
          ...
          <TableHead>
            <TableRow>
              <TableCell sx={{ bgcolor: "background.default" }}>
                {renderSort("visitId", "Visit ID")}
              </TableCell>
              <TableCell sx={{ bgcolor: "background.default" }}>
                {renderSort("startTime", "Start")}
              </TableCell>
            ...
            {rows.map((row) => (
              <TableRow key={row.visitId} hover>
                ...
                <TableCell align="right">
                  {formatNumber(row.loaded, 0)}
                </TableCell>
                <TableCell align="right">
                  {formatNumber(row.discharged, 0)}
                </TableCell>

                <TableCell align="right">
                  {formatNumber(row.restowCount, 0)}
                </TableCell>
                <TableCell align="right">
