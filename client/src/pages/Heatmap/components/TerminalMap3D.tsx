...
const REALISTIC_CONTAINERS = [0x1e293b, 0x0f172a, 0xca8a04, 0x991b1b, 0x0369a1, 0x064e3b, 0xd97706, 0xe2e8f0];
...
class TerminalScene {
  ...
  rmg = { craneX: to3D(260, TRENCH_CY).x, trolley: new THREE.Group(), spreader: new THREE.Group(), heldContainer: new THREE.Mesh() };
  ...
  buildWater() {
    ...
    const mat = new THREE.MeshStandardMaterial({ color: 0x18385e, roughness: 0.05, metalness: 0.9, flatShading: true, transparent: true, opacity: 0.97 });
  ...
  buildFuelStation() {
    ...
    const bldg = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.8, 0.5), new THREE.MeshStandardMaterial({ color: 0xf1f5f9, roughness: 0.8 }));
    bldg.position.set(0.9, 0.4, 0); bldg.castShadow = true; g.add(bldg);
    g.position.set(pos3.x, 0, pos3.z);
    this.scene.add(g);
    const stain = new THREE.Mesh(new THREE.PlaneGeometry(2.5, 1.8), new THREE.MeshBasicMaterial({ color: 0x111827, transparent: true, opacity: 0.25 }));
  ...
  buildArchitecture() {
    ...
    const flag = new THREE.Mesh(new THREE.PlaneGeometry(0.5, 0.28), new THREE.MeshBasicMaterial({ color: 0x1d4ed8, side: THREE.DoubleSide }));
    ...
    const dish = new THREE.Mesh(new THREE.SphereGeometry(0.2, 8, 4, 0, Math.PI), new THREE.MeshStandardMaterial({ color: 0xb0bec5, roughness: 0.4, metalness: 0.6 }));
    ...
    [320, 580].forEach((y, i) => {
      ...
      [-1.5, -0.4, 0.7].forEach(dx => {
        ...
        const dock = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.04, 0.4), new THREE.MeshStandardMaterial({ color: 0x374151, roughness: 0.9 }));
        dock.position.set(dx, 0.12, 1.72); wh.add(dock);
      });
      const stripe = new THREE.Mesh(new THREE.BoxGeometry(5.5, 0.2, 0.02), new THREE.MeshStandardMaterial({ color: 0x1d4ed8, roughness: 0.5 }));
    ...
    const ballast = new THREE.Mesh(new THREE.BoxGeometry(trenchWStr, 0.08, 20 * S), new THREE.MeshStandardMaterial({ color: 0x3f3f46, roughness: 0.9 }));
  ...
  buildTrain(zPos: number) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (i !== 1 && Math.random() > 0.2) {
        ...
        const cont = new THREE.Mesh(new THREE.BoxGeometry(0.60, 0.22, 0.23), new THREE.MeshStandardMaterial({ color: contColor, roughness: 0.7 }));
    ...
    train.userData = { state: 'INBOUND', speed: 0.05, targetX: craneAlignTargetX, startX: startX, stopTime: 0 };
  ...
  buildRMGCrane(xPos: number) {
    ...
    this.rmg.heldContainer = new THREE.Mesh(new THREE.BoxGeometry(0.60, 0.22, 0.23), new THREE.MeshStandardMaterial({ color: REALISTIC_CONTAINERS[0] }));
  ...
  buildTrucks() {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (Math.random() > 0.2) {
        const contColor = REALISTIC_CONTAINERS[i % REALISTIC_CONTAINERS.length];
        const cont = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.22, 0.48), new THREE.MeshStandardMaterial({ color: contColor, roughness: 0.6 }));
  ...
  buildDocks() {
    ...
    [[c.x - TERM_W * S / 2, northZ], [c.x + TERM_W * S / 2, northZ], [c.x - TERM_W * S / 2, southZ], [c.x + TERM_W * S / 2, southZ]].forEach(([lx, lz]) => {
  ...
  buildSTSCrane(svgX: number, svgY: number, rotDeg: number) {
    ...
    [[-0.07, 4.5, -jibL + 0.6], [-0.07, 4.5, 2.4]].forEach(([, , bz]) => {
      const cable = new THREE.Mesh(new THREE.BoxGeometry(0.015, 2.0, 0.015), new THREE.MeshStandardMaterial({ color: 0x64748b, roughness: 0.5, metalness: 0.8 }));
    ...
    stripes.forEach((col, si) => {
      const s = new THREE.Mesh(new THREE.BoxGeometry(0.105, 0.105, 0.4), new THREE.MeshStandardMaterial({ color: col, roughness: 0.4 }));
  ...
  buildShip(id: string, svgX: number, svgY: number, rotDeg: number, name: string, isTarget: boolean) {
    ...
    const wl = new THREE.Mesh(new THREE.BoxGeometry(L + 0.05, DR * 0.3, W + 0.05), new THREE.MeshStandardMaterial({ color: wlColor, roughness: 0.7 }));
    ...
    for (let i = 0; i < 5; i++) {
      const plate = new THREE.Mesh(new THREE.BoxGeometry(L + 0.01, 0.015, W + 0.01), new THREE.MeshStandardMaterial({ color: hullColor === 0x1e3a8a ? 0x1e3580 : 0x3d1503, roughness: 0.8 }));
    ...
    const deck = new THREE.Mesh(new THREE.BoxGeometry(L * 0.94, 0.06, W * 0.88), new THREE.MeshStandardMaterial({ color: 0x1a2232, roughness: 0.92, metalness: 0.1 }));
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        ...
        const cm = new THREE.Mesh(new THREE.BoxGeometry(cW * 0.88, stackH, cD * 0.88), new THREE.MeshStandardMaterial({ color: contColor, roughness: 0.65 }));
    ...
    const funnel = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.13, 0.5, 8), new THREE.MeshStandardMaterial({ color: funnelColor, roughness: 0.5 }));
  ...
  applyData(data: any, computedMaxBlock: string | null, targetBerthId: string, _maxBlockData?: { count: number; concentration: string }) {
    ...
    Object.entries(data.layout).forEach(([id, pos]: [string, any]) => {
      ...
      const pad = new THREE.Mesh(new THREE.BoxGeometry(bw, 0.12, bd), new THREE.MeshStandardMaterial({ color: padColor, roughness: 0.92 }));
      ...
      if (hasData) {
        ...
        const iMesh = new THREE.InstancedMesh(new THREE.BoxGeometry(cubeSize, cubeSize, cubeSize), new THREE.MeshStandardMaterial({ roughness: 0.7 }), count);
    ...
    heatGroups.forEach(({ id, cx, cz, bw, bd }) => {
      ...
      if (isHigh || isMed) {
        ...
        const pts = new THREE.Points(
          ...
          new THREE.PointsMaterial({ color: new THREE.Color(col), size: 0.14, transparent: true, opacity: 0.85, sizeAttenuation: true, depthWrite: false, blending: THREE.NormalBlending })
        );
...
export default function TerminalMap3D({
  ...
  return (
    ...
    >
      ...
      {hoveredBlock && (
        <Box sx={{
          position: "absolute", top: "50%", left: 24, transform: "translateY(-50%)", zIndex: 10, px: 2, py: 1.4,
